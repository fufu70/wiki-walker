...
export class InputFactory {
  ...
  static removeTouchpad() {
		if (!InputFactory.mobileInput) {
			return;
		}
		InputFactory.mobileInput.remove();
		delete InputFactory.mobileInput;	
	}
	...
	addMobileInput(input) {
		const mobileParams = {
			...
			aPressed: () => {
				...
				input.lastTextKeys.push('Space');
			},
			bPressed: () => {
				input.keys['Enter'] = true;
				input.lastTextKeys.push('Enter');
			},
			xPressed: () => {
				input.keys['Escape'] = true;
				input.lastTextKeys.push('Escape');
			},
			yPressed: () => {
				input.keys['ShiftLeft'] = true;
				input.lastTextKeys.push('ShiftLeft');
			},
			aReleased: () => {
		...
		return new MobileInput(mobileParams);
	}
