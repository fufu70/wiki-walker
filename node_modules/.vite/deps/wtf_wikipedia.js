...
var getResult = function(data, options = {}) {
  var _a;
  if (!((_a = data == null ? void 0 : data.query) == null ? void 0 : _a.pages) || !(data == null ? void 0 : data.query) || !data) {
    return null;
  }
...
// node_modules/wtf_wikipedia/src/_data/stubs.js
var stubs_default = [
  "aboç",
  "ahurhire",
  "aizmetnis",
...
var isDisambig = function(doc) {
  let templates15 = doc.templates().map((tmpl) => tmpl.json());
  let found = templates15.find((obj) => {
  ...
  let notDisamb = templates15.find((obj) => notDisambig.hasOwnProperty(obj.template));
  if (notDisamb) {
    return false;
  }
...
// node_modules/wtf_wikipedia/src/01-document/isStub.js
var allStubs = new Set(stubs_default);
var isStub = function(doc) {
  let templates15 = doc.templates().map((tmpl) => tmpl.json());
  return templates15.some((t) => {
    let name = t.template || "";
...
var external_links = function(links2, str) {
...
var internal_links = function(links2, str) {
...
var parse_links = function(str) {
  let links2 = [];
  links2 = external_links(links2, str);
  links2 = internal_links(links2, str);
  if (links2.length === 0) {
    return void 0;
  }
  return links2;
...
var toJSON2 = function(section, options) {
  ...
  if (options.templates === true) {
    let templates15 = section.templates().map((tmpl) => tmpl.json());
    if (templates15.length > 0) {
      data.templates = templates15;
...
var getLinks = function(data) {
  let wiki = data.text;
  let links2 = parse_default(wiki) || [];
  data.links = links2.map((link) => {
  ...
  links(n) {
    ...
    this.data.forEach((r) => {
      Object.keys(r).forEach((k) => {
        links2 = links2.concat(r[k].links());
      });
    });
    if (typeof n === "string") {
      n = n.charAt(0).toUpperCase() + n.substring(1);
      let link = links2.find((o) => o.page() === n);
      return link === void 0 ? [] : [link];
    }
    return links2;
  },
...
var methods6 = {
  ...
  links(clue) {
    ...
    this.lines().forEach((s) => {
      links2 = links2.concat(s.links());
    });
    if (typeof clue === "string") {
      clue = clue.charAt(0).toUpperCase() + clue.substring(1);
      let link = links2.find((o) => o.page() === clue);
      return link === void 0 ? [] : [link];
    }
    return links2;
  },
...
// node_modules/wtf_wikipedia/src/template/custom/text-only/shorthand-link.js
var templates3 = {};
var arr = [
  // ships
  "mv",
...
var links = ["no redirect", "tl-r", "template link no redirect", "redirect?", "subatomic particle", "auto link", "bl"];
links.forEach((word) => {
  templates3[word] = (tmpl) => {
    let data = toJSON_default(tmpl, ["page", "text"]);
...
var shorthand_link_default = templates3;
...
var functions_default = {
  ...
  "literal translation": (tmpl) => {
    let arr2 = toJSON_default(tmpl).list || [];
    arr2 = arr2.map((str) => `'${str}'`);
    return "lit. " + arr2.join(" or ");
  },
  overset: (tmpl) => {
    let data = toJSON_default(tmpl, ["over", "base"]);
    return [data.over || "", data.base || ""].join(" ");
  },
  underset: (tmpl) => {
    let data = toJSON_default(tmpl, ["under", "base"]);
    return [data.base || "", data.under || ""].join(" ");
  },
  ceil: (tmpl) => {
    let data = toJSON_default(tmpl, ["txt"]);
    return `⌈${data.txt}⌉`;
  },
  floor: (tmpl) => {
    let data = toJSON_default(tmpl, ["txt"]);
  ...
  "vol.": (tmpl) => {
    let data = toJSON_default(tmpl, ["n"]);
    return `vol. ${data.n}`;
  },
  rp: (tmpl) => {
    let data = toJSON_default(tmpl, ["page"]);
    if (data.pages) {
  ...
  bra: (tmpl) => {
    let data = toJSON_default(tmpl, ["a"]);
    return `⟨${data.a || ""}|`;
  },
  ket: (tmpl) => {
    let data = toJSON_default(tmpl, ["a"]);
  ...
  "bra-ket": (tmpl) => {
    let data = toJSON_default(tmpl, ["a", "b"]);
    return `⟨${data.a || ""}|${data.b || ""}⟩`;
  },
  braket: (tmpl) => {
    let data = toJSON_default(tmpl, ["sym", "a", "b"]);
    if (data.sym === "bra") {
      return `⟨${data.a}|`;
  ...
  pars: (tmpl) => {
    let data = toJSON_default(tmpl, ["text", "s"]);
    return `(${data.text || ""})`;
  },
  circumfix: (tmpl) => {
    let data = toJSON_default(tmpl, ["text"]);
  ...
  fluc: (tmpl) => {
    let data = toJSON_default(tmpl, ["val", "type"]);
  ...
  subsup: (tmpl) => {
    let data = toJSON_default(tmpl, ["symbol", "subscript", "superscript"]);
    return `${data.symbol || ""} ${data.subscript || ""} ${data.superscript || ""}`;
  },
  su: (tmpl) => {
    let data = toJSON_default(tmpl, ["p", "b"]);
    return `${data.p || ""} ${data.b || ""}`;
  },
  precision: (tmpl) => {
    let data = toJSON_default(tmpl, ["num"]);
    let num = data.num || "";
  ...
  intmath: (tmpl) => {
    let data = toJSON_default(tmpl, ["sign", "subscript", "superscript"]);
    const signs = {
      int: "∫",
      iint: "∬",
      iiint: "∭",
  ...
  ldelim: (tmpl) => {
    let data = toJSON_default(tmpl, ["a", "b", "sub", "sup"]);
  ...
  round: (tmpl) => {
    let data = toJSON_default(tmpl, ["val", "decimals"]);
    let n = Number(data.val);
    return Math.round(n) || "";
  },
  rounddown: (tmpl) => {
  ...
  parity: (tmpl) => {
    let data = toJSON_default(tmpl, ["val", "even", "odd"]);
  ...
  hexadecimal: (tmpl) => {
    let data = toJSON_default(tmpl, ["val"]);
    let n = Number(data.val);
    if (!n) {
      return data.val;
    }
  ...
  decimal2base: (tmpl) => {
    let data = toJSON_default(tmpl, ["n", "radix"]);
    let n = Number(data.n);
  ...
  ifnotempty: (tmpl) => {
    let data = toJSON_default(tmpl, ["cond", "a", "b"]);
    if (data.cond) {
      return data.a;
    }
    return data.b;
  ...
  ifnumber: (tmpl) => {
    let data = toJSON_default(tmpl, ["n", "yes", "no"]);
  ...
  "percent and number": (tmpl) => {
    let data = toJSON_default(tmpl, ["number", "total", "decimals"]);
  ...
  music: (tmpl) => {
    let data = toJSON_default(tmpl, ["glyph"]);
    let glyphs = {
      flat: "♭",
      b: "♭",
      sharp: "♯",
  ...
  simplenuclide: (tmpl) => {
    let data = toJSON_default(tmpl, ["name", "mass"]);
    return `[[${data.name}|${data.mass || ""}${data.name}]]`;
  },
  "font color": (tmpl) => {
    let data = toJSON_default(tmpl, ["fg", "bg", "text"]);
  ...
  "colored link": (tmpl) => {
    let data = toJSON_default(tmpl, ["color", "title", "text"]);
    return `[[${data.title}|${data.text || data.title}]]`;
  },
  nftu: (tmpl) => {
    let data = toJSON_default(tmpl, ["age", "team"]);
    return `${data.team} U${data.age}`;
  },
  tls: (tmpl) => {
    let data = toJSON_default(tmpl, ["name", "one", "two"]);
...
// node_modules/wtf_wikipedia/src/template/custom/text-only/abbreviations.js
var shorthands = [
...
var fns = shorthands.reduce((h, a) => {
  let [name, out] = a;
  h[name] = (tmpl) => {
    let { first } = toJSON_default(tmpl, ["first"]);
    if (first || first === 0) {
      return out + (first || "");
    }
    return out;
...
var abbreviations_default2 = fns;
...
var text_only_default = Object.assign({}, hardcoded_default, easy_inline_default, shorthand_link_default, functions_default, languages_default2, abbreviations_default2, flags_default2, table_cell_default);
...
// node_modules/wtf_wikipedia/src/template/custom/data-only/dummy-templates.js
var templates9 = {};
var dummies = ["citation needed"];
dummies.forEach((name) => {
  templates9[name] = (tmpl, list2) => {
    list2.push(toJSON_default(tmpl));
    return "";
  };
});
var dummy_templates_default = templates9;
...
var functions_default2 = {
  ...
  "sister project links": (tmpl, list2) => {
    ...
    Object.keys(sisterProjects).forEach((k) => {
      if (data.hasOwnProperty(k) === true) {
        links2[sisterProjects[k]] = data[k];
      }
    });
    let obj = {
  ...
  r: (tmpl, list2) => {
    let obj = toJSON_default(tmpl, ["name"]);
    obj.template = "citation";
    list2.push(obj);
    return "";
  }
...
templates10 = Object.assign(templates10, dummy_templates_default, id_name_default, id_title_default, functions_default2);
var data_only_default = templates10;
...
`;
  ...
  ReptileDatabase: (tmpl, list2) => {
    let obj = toJSON_default(tmpl, ["taxon", "genus", "species", "abruf", "pure_url"]);
    list2.push(obj);
    let str = "";
  ...
  GEOnet3: (tmpl, list2) => {
    let obj = toJSON_default(tmpl, ["ufi", "name"]);
    list2.push(obj);
    return `GEOnet3 can be found at [[GEOnet Names Server]], at [http://geonames.nga.mil/namesgaz/ this link]`;
  },
  "poem quote": (tmpl, list2) => {
    let obj = toJSON_default(tmpl, ["text", "char", "sign", "source", "title"]);
    list2.push(obj);
    let out = obj.text || "";
...
var functions_default3 = templates11;
...
Object.keys(codes).forEach((k) => {
  templates12[k] = parseCurrency;
});
var currency_default = templates12;
...
var toTextBritish = function(date) {
  let str = String(date.year || "");
  if (date.month !== void 0 && months.hasOwnProperty(date.month) === true) {
    if (date.date === void 0) {
      str = `${months[date.month]} ${date.year}`;
...
var dates_default = {
  ...
  "birth date": (tmpl, list2) => {
    let obj = toJSON_default(tmpl, ["year", "month", "date"]);
    list2.push(obj);
...
var templates14 = Object.assign({}, text_only_default, data_only_default, text_and_data_default);
Object.keys(aliases_default).forEach((k) => {
  if (templates14[aliases_default[k]] === void 0) {
    console.error(`Missing template: '${aliases_default[k]}'`);
  }
  templates14[k] = templates14[aliases_default[k]];
});
var custom_default = templates14;
...
var process = function(section, doc) {
  ...
  let { infoboxes, references, templates: templates15 } = sortOut_default(list2, domain);
  ...
  section._templates = section._templates.concat(templates15);
  section._wiki = wiki;
};
...
var parseElection = function(catcher, doc) {
  catcher.text = catcher.text.replace(/\{\{election box begin([\s\S]+?)\{\{election box end\}\}/gi, (tmpl) => {
    ...
    let templates15 = data._templates.map((t) => t.json());
    let start = templates15.find((t) => t.template === "election box") || {};
    let candidates = templates15.filter((t) => t.template === "election box candidate");
    let summary = templates15.find((t) => t.template === "election box gain" || t.template === "election box hold") || {};
